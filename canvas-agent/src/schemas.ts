...
const assetCategorySchema = z.enum(["character", "environment", "prop", "material", "other"]);
const agentCandidateCategorySchema = z.enum(["environment", "prop", "material", "other"]);
const projectIdSchema = z.string().min(1).optional();
const projectCandidateSchema = z.object({ unitId: z.string().optional(), shotId: z.string().optional(), name: z.string().min(1), category: agentCandidateCategorySchema, details: recordSchema.optional() });
...
const writePreconditionsSchema = z.object({
    expectedRevision: z.number().int().nonnegative().optional(),
    expectedStateHash: z.string().min(1).optional(),
});
...
export const toolInputSchemas = {
    ...
    canvas_validate_ops: z.object({ ops: z.array(canvasOpSchema) }).merge(writePreconditionsSchema),
    canvas_get_selection: z.object({}).passthrough(),
    canvas_export_snapshot: z.object({}).passthrough(),
    canvas_apply_ops: z.object({ ops: z.array(canvasOpSchema) }).merge(writePreconditionsSchema),
    canvas_create_workflow: workflowSchema.merge(writePreconditionsSchema),
    canvas_create_node: z.object({ nodeType: nodeTypeSchema, title: z.string().optional(), x: z.number().optional(), y: z.number().optional(), width: z.number().optional(), height: z.number().optional(), metadata: recordSchema.optional() }).merge(writePreconditionsSchema),
    canvas_create_text_node: z.object({ text: z.string().optional(), x: z.number().optional(), y: z.number().optional(), title: z.string().optional(), width: z.number().optional(), height: z.number().optional() }).merge(writePreconditionsSchema),
    canvas_create_text_nodes: z.object({ items: z.array(textNodeSchema).min(1), x: z.number().optional(), y: z.number().optional(), gap: z.number().optional(), direction: z.enum(["row", "column"]).optional() }).merge(writePreconditionsSchema),
    canvas_create_image_prompt_flow: z.object({ prompt: z.string(), x: z.number().optional(), y: z.number().optional(), autoRun: z.boolean().optional() }).merge(generationOptionsSchema).merge(writePreconditionsSchema),
    canvas_create_generation_flow: generationFlowSchema.extend({ mode: generationModeSchema.optional(), autoRun: z.boolean().optional() }).merge(generationOptionsSchema).merge(writePreconditionsSchema),
    canvas_generate_text: generationFlowSchema.merge(generationOptionsSchema).merge(writePreconditionsSchema),
    canvas_generate_image: generationFlowSchema.merge(generationOptionsSchema).merge(writePreconditionsSchema),
    canvas_generate_video: generationFlowSchema.merge(generationOptionsSchema).merge(writePreconditionsSchema),
    canvas_generate_audio: generationFlowSchema.merge(generationOptionsSchema).merge(writePreconditionsSchema),
    canvas_update_node: z.object({ id: z.string(), patch: recordSchema.optional(), metadata: recordSchema.optional() }).merge(writePreconditionsSchema),
    canvas_update_node_text: z.object({ id: z.string(), text: z.string(), title: z.string().optional() }).merge(writePreconditionsSchema),
    canvas_move_nodes: z.object({ items: z.array(z.object({ id: z.string(), x: z.number().optional(), y: z.number().optional(), dx: z.number().optional(), dy: z.number().optional() })).min(1) }).merge(writePreconditionsSchema),
    canvas_resize_node: z.object({ id: z.string(), width: z.number(), height: z.number(), freeResize: z.boolean().optional() }).merge(writePreconditionsSchema),
    canvas_delete_nodes: z.object({ ids: z.array(z.string()).min(1) }).merge(writePreconditionsSchema),
    canvas_connect_nodes: z.object({ connections: z.array(z.object({ fromNodeId: z.string(), toNodeId: z.string(), fromHandleId: z.string().optional(), toHandleId: z.string().optional() })).min(1) }).merge(writePreconditionsSchema),
    canvas_select_nodes: z.object({ ids: z.array(z.string()) }).merge(writePreconditionsSchema),
    canvas_set_viewport: z.object({ viewport: viewportSchema }).merge(writePreconditionsSchema),
    canvas_run_generation: z.object({ nodeId: z.string(), mode: generationModeSchema.optional(), prompt: z.string().optional(), retry: z.boolean().optional(), clientOperationId: z.string().min(1).max(120).optional(), idempotencyKey: z.string().min(1).max(120).optional() }).merge(writePreconditionsSchema),
...
export const toolDescriptions: Record<ToolName, string> = {
    ...
    project_extract_asset_candidates: "将分镜识别出的场景、道具、素材或其他需求登记为待确认资产候选；角色只在剧情章节提取，不通过此工具重复创建。",
